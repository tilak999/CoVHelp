...
function InfoCard({ data }) {
  ...
  return (
    <div className="shadow-sm border border-gray-200 rounded-lg px-4 py-2 my-2 hover:bg-gray-50">
      ...
      <div className="text-sm text-bold text-indigo-600">
  ...
  return (
    <div className="bg-blue-200 min-h-screen p-2">
      ...
      <div className="container mx-auto bg-white rounded-lg p-4">
        ...
        <br />
        <div className="my-4 text-red-500 font-bold text-center">
          <a href="https://forms.gle/JYmweJ8hBY6VYkgL6" target="_blank">
            <svg
              ...
              height="16"
              fill="currentColor"
              className="inline-block text-base animate-bounce"
              viewBox="0 0 16 16"
            >
              <path
                fillRule="evenodd"
          ...
          <button className="font-bold px-4" onClick={() => psycologicalHelp()}>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="16"
              fill="currentColor"
              viewBox="0 0 16 16"
              className="inline-block text-base mx-2"
            >
          ...
          <a
            href="/plasmaDonate"
            className="font-bold px-2 my-3 inline-block"
            target="_blank"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
        ...
        <div className="mt-4 mb-2 flex">
          <div>
            <div htmlFor="cars" className="mb-2 font-bold text-sm">
              City:
            </div>
            <select
        ...
        <div className="py-1">
          <button
            className="px-2 py-1 bg-indigo-500 text-white rounded-md w-full"
            onClick={() => getData()}
          >
            Search
          </button>
        ...
        {data != null ? (
          <div className="my-6">
            <p className="text-sm p-3 bg-blue-100 text-blue-800 rounded">
              We try our best to keep the list updated but if contact ran out of
              resources or not connecting please try with next entry in the
              list.
            </p>
            <h1 className="font-bold text-gray-600 my-4 text-sm">
              Showing results for: {filter || appData.filters[0]} in{" "}
              {city || appData.cities[0]}
            </h1>
            {data.map((data) => (
              <InfoCard data={data} key={data.mobile} />
            ))}
            {data.length == 0 ? (
              <div>Sorry, no information available at this time.</div>
            ) : (
              ""
            )}
          </div>
        ) : (
          <div className="bg-gray-50 text-center p-4 rounded-xl text-gray-700 my-4">
            Use search to find help
          </div>
        )}
      </div>

      {appData.notice && appData.notice != "" ? (
        <div className="container mx-auto bg-white rounded-lg border-l-4 border-yellow-600 p-4 mt-4">
          ...
          <p
            ...
            dangerouslySetInnerHTML={{ __html: appData.notice }}
          ></p>
        </div>
      ) : (
        ""
      ...
      <div className="container mx-auto bg-white rounded-lg border-l-4 border-green-600 p-4 mt-4">
        ...
        <p className="my-4">
          In this tough time, we need each other to help ourselves. We welcome
          any help and suggestions to make this listing helpful.
        </p>
        <div className="rounded-lg p-3 bg-green-200 float-left my-1 mr-3">
          <svg
            height="15pt"
            width="15pt"
        ...
        <p>
          <b className="text-sm">Mrityunjay Patidar</b>
          <br />
          +917869294816
        </p>
      </div>
